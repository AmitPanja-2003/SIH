...
import "../CSS/AddPatientBed.css";
import Hospital from "../models/hospital";
import { useLocation } from 'react-router-dom';
import { useNavigate } from 'react-router-dom';

// Uncomment and adjust base URL if necessary
...
const AddPatientBed = () => {
  ...
  const fetchBedData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/addpatientbed/beds",{hospitalId:hospitalId});  // Adjust the endpoint
  ...
  const fetchPatientData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/addpatientbed/patients");  // Adjust the endpoint
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5000/addpatientbed/patients", dataToSubmit);    // Adjust the endpoint
  ...
  const updateBedAvailability = async (bedType, newCount) => {
    try {
      await axios.put("http://localhost:5000/addpatientbed/beds/update", {    // Adjust the endpoint
        bedType,
  ...
  const handleCheckout = async (patientId, bedType) => {
    try {
      await axios.post(`http:/localhost:5000/addpatientbed/patients/checkout/${patientId}`);  // Adjust the endpoint
